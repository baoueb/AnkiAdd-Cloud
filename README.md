...
- ✅ Background “desktop puller” (macOS) that can run automatically  
...
## Privacy & data-sharing (read this)
...
1. Quickstart (10 minutes)
2. How it works (architecture)
3. AWS setup (step-by-step)
4. Backend code (Lambda) + API contract
5. Desktop puller setup (macOS) + auto-run
6. Configure AnkiAdd (Cloud Base URL + token)
7. Troubleshooting + FAQ
8. Roadmap (hardening & polish)


## Chunk 2 — Quickstart (10 minutes)
...
The full Lambda code is provided later in the README.
For Quickstart, you just need the API running.
...
curl -i -H "Authorization: Bearer $TOKEN" \
  "$INVOKE_URL/v1/notes/pending?limit=5"
...
curl -i -H "Authorization: Bearer $TOKEN" \
  "$INVOKE_URL/v1/notes/status?since=0"
...
Next: How it works (architecture + privacy)

Continue to the next section to understand exactly what data is uploaded, where it is stored, and how results come back to your phone.

## Chunk 3 — How it works (architecture + privacy)
...
Cloud Sync is a simple “inbox” system:

1. **AnkiAdd (iPhone/iPad)** uploads your card data to your cloud API.
2. The **Desktop Puller** (running on your computer) polls the cloud for pending notes.
3. The Desktop Puller calls **AnkiConnect** (localhost) to add the note into **Anki Desktop**.
4. The Desktop Puller posts the result back to the cloud (**Added / Duplicate / Failed**).
5. **AnkiAdd** polls the cloud for updates and shows the final status on your phone.
...
Duplicates are detected on the desktop side (AnkiConnect):
	...
	•	Puller may call findNotes to locate existing notes
	•	Puller reports back DUPLICATE with duplicateNoteIds (when available)
...
Your phone does not talk to Anki Desktop directly. Instead:
	...
	•	When desktop finishes, it posts results to the cloud
...
## Chunk 4 — AWS setup (step-by-step)
...
Without an index, /pending becomes slower as your table grows.
With this index, /pending stays fast.
...
Lambda → Configuration tab → Environment variables → Edit:
...
AWS Console top-right → click your account → copy Account ID
...
Replace the entire file with the code provided in this repo:
	•	lambda_function.py

Then click Deploy.

✅ After deploying, your Lambda is ready to receive traffic.
...
Attach each route to the same Lambda integration.

Deploy

HTTP APIs often auto-deploy to $default.
	•	API Gateway → Stages → $default
	•	Confirm Auto-deploy is enabled
(If not, click Deploy)

Copy the Invoke URL

You will see an Invoke URL like:

https://xxxx.execute-api.ap-northeast-1.amazonaws.com

This is your Cloud Base URL (used in AnkiAdd + Desktop Puller).
...
## Chunk 5 — Backend code (Lambda) + API contract
...
curl -sS -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{"notes":[{"clientNoteId":"11111111-1111-1111-1111-111111111111","createdAt":"2026-03-01T00:00:00Z","deck":"Default","model":"Basic","fields":{"Expression":"階段","MainDefinition":"test"},"tags":["AnkiAdd"]}]}' \
  "$INVOKE_URL/v1/notes/batch"
...
## Chunk 6 — Desktop Puller (script + LaunchAgent auto-run)
...
Puller says “AnkiConnect not reachable”
	•	Open Anki Desktop
	•	Verify http://127.0.0.1:8765 is responding
	•	Reinstall AnkiConnect
...
export ANKIADD_HTTP_TIMEOUT="25"
	...
	•	Ensure Lambda timeout is 10–15 seconds and memory 512MB

LaunchAgent runs but does nothing
	•	Confirm the python path in ProgramArguments
	•	Confirm the script path is correct
	•	Check logs in /tmp/ankiadd-puller.err.log

⸻

## Chunk 7 — Configure AnkiAdd + troubleshooting + FAQ

This section explains how to configure AnkiAdd for Cloud Sync, what each status means, and how to fix common issues.

---

## App configuration (AnkiAdd)

Open **AnkiAdd → Settings → Cloud** and set the following:
...
This makes the Sync button upload your queue to the cloud instead of using AnkiConnect directly.
...
If you can choose a polling interval, recommended:
	•	10–20 seconds
...
What must happen next:
	•	The Desktop Puller must be running
...
Common causes:
	...
	•	Missing DynamoDB permissions
	•	Decimal JSON serialization issue (fixed by custom encoder in this repo)
...
This usually means:
	...
	•	Or the puller is running multiple copies and conflicts

Fix:
	•	Restart the puller
	•	Ensure only one puller instance is running
	•	(Hardening suggestion) Add a “processing timeout” rule in Lambda (Phase 7)
...
Your phone only talks to the cloud API. Your desktop only talks to the cloud API and localhost AnkiConnect.
...
However:
	•	Your card content is stored in your AWS account.
	•	Anyone with your token can submit notes to your API.
...
Typically low for personal use, but depends on how much you:
	•	Upload notes
	•	Poll status
	•	Keep items stored
...
Yes, but the LaunchAgent is macOS-specific. You would run the puller:
	•	manually in a terminal
	•	or via Windows Task Scheduler (not covered in this README yet)

“Can I share this setup with friends?”

Technically yes, but you should add:
	•	per-user authentication (not a single shared token)
	•	better rate-limits
	•	logging/admin tools
	•	idempotency keys
(see Phase 7 ideas)
